...
def run():
    data_months = (
        date(year, month, 1) for year in range(2002, 2012) for month in range(1, 13)
    )
    request_params = {
        "timeframe": 1,  # Daily
        "Prov": "BC",
        ...
        for data_month in data_months:
            ...
            for record in ec_data.findall("stationdata"):
                ...
                while timestamp.date() > yvr_data["date"]:
                    yvr_data = get_yvr_line(yvr_file, START_YEAR).next()
                if DUMP_HOURLY_RESULTS:
                    write_hourly_line(timestamp, weather_desc, yvr_data, hourly_file)
...
def write_hourly_line(timestamp, weather_desc, yvr_data, hourly_file):
    result_line = "{0:%Y-%m-%d %H:%M:%S} {1} {2}\n".format(
        timestamp, weather_desc, yvr_data["hourly_cfs"][timestamp.hour]
    )
...
def calc_mapping_averages(mapping):
    for weather_desc, months in mapping.iteritems():
        ...
        if total_observations > AVERAGING_THRESHOLD:
            log.info(
                'using monthly averaging for {0} "{1}" observation(s)'.format(
                    total_observations, weather_desc
                )
            )
            ...
            log.info(
                'using all value averaging for {0} "{1}" observation(s)'.format(
                    total_observations, weather_desc
                )
            )
            mapping[weather_desc] = [
