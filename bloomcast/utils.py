...
class Config(object):
    ...
    def load_config(self, config_file):
        ...
        self.run_start_date = infile_dict["run_start_date"].replace(
        ...
        timeseries_keys = (
            ...
            "std_chem_ts_outfile user_chem_ts_outfile".split()
        )
        ...
        profiles_keys = (
            ...
            "halocline_outfile "
            "Hoffmueller_profiles_outfile user_Hoffmueller_profiles_outfile".split()
        )
    ...
    def _load_results_config(self, config_dict):
        ...
        for key, value in config_dict.items():
            if key == "push_to_web":
                ...
                self.results.__dict__[key] = pathlib.Path(value)
    ...
    def _load_meteo_config(self, config_dict, infile_dict):
        ...
        self.climate.meteo.__dict__.update(config_dict["climate"]["meteo"])
    ...
    def _read_yaml_file(self, config_file):
        ...
        log.debug("data structure read from {}".format(config_file))
        return config
    ...
    def _read_SOG_infile(self, yaml_file):
        ...
        infile_values = {
            ...
            "profiles_results.profile_file_base": ("profiles_outfile_base"),
            "profiles_results.user_profile_file_base": ("user_profiles_outfile_base"),
            "profiles_results.halocline_file": ("halocline_outfile"),
            "profiles_results.hoffmueller_file": ("Hoffmueller_profiles_outfile"),
        ...
        log.debug(
            "run start date, time step, and output file names read from {}".format(
                yaml_file
            )
        )
...
class ForcingDataProcessor(object):
    ...
    def _trim_data(self, qty):
        ...
        """
        while self.data[qty]:
            if any([self._valuegetter(data[1]) for data in self.data[qty][-24:]]):
                break
            else:
    ...
    def patch_data(self, qty):
        ...
        for i, data in enumerate(self.data[qty]):
            if self._valuegetter(data[1]) is None:
                ...
                gap_end = i
                log.debug("{qty} data patched for {date}".format(qty=qty, date=data[0]))
                gap_count += 1
        ...
        * Data requires special handling for interpolation; e.g. wind
        ...
        if gap_hours > 11:
            log.warning(
                ...
                "by linear interpolation".format(
                    qty=qty, gap_start=self.data[qty][gap_start][0]
                )
            )
...
class ClimateDataProcessor(ForcingDataProcessor):
    ...
    def _get_data_months(self):
        ...
        data_months = [
            datetime.date(this_year, month, 1) for month in range(1, today.month + 1)
        ]
        if self.config.run_start_date.year != this_year:
            last_year = self.config.run_start_date.year
            data_months = [
                datetime.date(last_year, month, 1) for month in range(1, 13)
            ] + data_months
        return data_months
